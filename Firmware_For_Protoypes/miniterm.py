...
---       %(ulb)-8s Upload binary file (prompt will be shown)
--- Toggles:
                        ...
                        if filename:
                            try:
                                ...
                                while True:
                                    ...
                                    if ((file.tell()%256)==0):
                                        time.sleep(0.05);
                                        sys.stderr.write('%d\r\n'%(file.tell()/256))   # Progress indicator.
                                    # Wait for output buffer to drain.
                                    self.serial.flush()
                                if (byte_counter !=0):
                                   sys.stderr.write('\n--- Byte remaining %d \n' % (256-byte_counter))
                                   for i in range(byte_counter,256):
